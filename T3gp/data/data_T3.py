...
if theoryid in {200, 208}:
    fk_p = load_fktable(loader.check_fktable(setname="BCDMSP", theoryID=theoryid, cfac=()))
    fk_d = load_fktable(loader.check_fktable(setname="BCDMSD", theoryID=theoryid, cfac=()))
...
kinematics = np.column_stack((
    merged_df["x"].to_numpy(),
    merged_df["q2"].to_numpy()
))
